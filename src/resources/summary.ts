...
export function registerSummaryResource(server: McpServer, repoRoot: string) {
  server.registerResource(
    ...
    'git://repo/summary',
    {
      description: 'Repository snapshot: branch, last commit, total commits, active contributors, top languages, and age.',
      mimeType: 'text/plain',
    },
    async () => {
      const parts: string[] = [];
      ...
      // Last commit
      const { stdout: lastCommit } = await gitExec(
      ...
      // Total commits
      const { stdout: totalCommits } = await gitExec(
        ['rev-list', '--count', 'HEAD'],
        { cwd: repoRoot },
      );
      ...
      const contributors = await gitLines(
        ['log', '--format=%aN', '--since=90 days ago'],
        { cwd: repoRoot },
      );
      ...
      const allContributors = await gitLines(
        ['log', '--format=%aN'],
        { cwd: repoRoot },
      );
      ...
      const fileLines = await gitLines(
        ['ls-files'],
        { cwd: repoRoot },
      );
      ...
      // Remote
      const { stdout: remote } = await gitExec(
